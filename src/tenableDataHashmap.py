...
for scanName in scanNameList:
    ...
    for index,historyRow in enumerate(historyList):

        sqlHistory = []
        if index == 1: #Only need latest entry
            break
        ...
        cursor.execute("SELECT * FROM "+sqlScanName+"_HISTORY")
    ...
    for index,creation_date in enumerate(sqlHistoryDates):
        if index < 2:
            ...
            connection.commit()
    print()
print("Hash map for Tenable complete...")
